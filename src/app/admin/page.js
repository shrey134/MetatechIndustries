...
export default function AdminPage() {
  ...
  const [sheetsExportUrl, setSheetsExportUrl] = useState(null);
  const [tokenClient, setTokenClient] = useState(null);

  // Ref to always hold the current secret — fixes stale closure in GIS callback
  ...
  const handleExport = (format) => {
    ...
    if (format === 'google-sheets') {
      ...
      // Set callback right before requesting token (recommended GIS pattern)
      tokenClient.callback = async (response) => {
        ...
        console.log('[GIS] Access token received:', accessToken ? 'YES (length: ' + accessToken.length + ')' : 'NO');
        console.log('[GIS] Admin secret available:', secretRef.current ? 'YES' : 'NO');
        console.log('[GIS] Triggering export...');

        await executeExport('google-sheets', accessToken);
      };
  ...
  const executeExport = async (format, accessToken = null) => {
    setIsExporting(true);
    ...
    try {
      ...
      if (format === 'google-sheets') {
        ...
        if (data.success && data.url) {
          console.log('[Export] Sheet URL:', data.url);
          setSheetsExportUrl(data.url); // Show link in UI — popup-blocker proof
        } else {
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 md:p-10 font-work-sans text-slate-800">
      <div className="max-w-[1400px] mx-auto">
        ...
        {/* Google Sheets Export Ready Banner */}
        {sheetsExportUrl && (
          <div className="mb-6 bg-emerald-50 border border-emerald-200 rounded-sm p-4 flex items-center justify-between gap-4 animate-in fade-in slide-in-from-top-2 duration-300">
            <div className="flex items-center gap-3">
              <span className="material-symbols-outlined text-emerald-600 text-xl">check_circle</span>
              <div>
                <p className="text-sm font-black text-emerald-800">Google Sheet is ready!</p>
                <p className="text-xs text-emerald-600 font-medium mt-0.5">Your data has been exported. Click to open in Google Sheets.</p>
              </div>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              <a
                ...
                className="bg-emerald-600 text-white px-4 py-2 text-xs font-black uppercase tracking-widest hover:bg-emerald-700 transition-colors flex items-center gap-2"
              >
                Open Sheet <span className="material-symbols-outlined text-sm">open_in_new</span>
              </a>
              <button
                onClick={() => setSheetsExportUrl(null)}
