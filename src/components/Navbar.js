      ...
      >
        ...
        <div className="flex flex-wrap border-b border-[#eee] bg-white p-1">
          {tabs.map((tab, idx) => (
            <button
              ...
              className={`px-3 py-3 text-[15px] font-medium tracking-tight transition-all duration-200 border-b-2 m-0.5 rounded-sm whitespace-nowrap ${activeGroup === idx
                ? 'text-[#1a3c5a] border-[#1a3c5a] bg-slate-50'
                : 'text-[#64748b] border-transparent hover:text-[#1a3c5a] hover:bg-slate-50/50'
                }`}
            >
        ...
        <div className="flex flex-col md:flex-row overflow-y-auto max-h-[70vh] category-scroll bg-white">
              ...
              {tabs[activeGroup].categories.map((category, cIdx) => (
                <div key={cIdx} className="border border-[#f1f5f9] rounded-sm overflow-hidden">
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-4 text-left transition-colors duration-200 ${activeAccordion === category.name
                      ? 'bg-slate-50 text-[#1a3c5a] border-l-4 border-l-[#1a3c5a]'
                      : 'hover:bg-slate-50/50 text-[#475569]'
                      }`}
                  >
                    <span className="font-medium text-[15px] tracking-tight capitalize flex items-center gap-2">
                      {category.icon && (
                        <span className="material-symbols-outlined text-[20px] leading-none text-[#1E6FA8]">
                          {category.icon}
                        </span>
                      )}
                      {category.name}
                  ...
                  {/* Accordion Content */}
                  <div className={`transition-all duration-300 ease-in-out overflow-hidden ${activeAccordion === category.name ? 'max-h-[800px] opacity-100' : 'max-h-0 opacity-0'
                    }`}>
                    <div className="p-4 bg-white grid grid-cols-1 gap-1">
                      ...
                      <Link
                        ...
                        className="mt-4 inline-flex items-center gap-1.5 text-[12px] font-medium text-[#1a3c5a] hover:underline px-3 opacity-80 hover:opacity-100"
                        onClick={() => setIsProductsOpen(false)}
                      >
          ...
          <div className="w-full md:w-1/2 p-12 bg-slate-50/20 flex flex-col justify-center items-start text-left">
            <h3 className="text-[#1a3c5a] text-[18px] font-medium mb-4 flex items-center gap-3">
              <span className="w-8 h-[1px] bg-[#1a3c5a]"></span>
              {toSentenceCase(tabs[activeGroup].title)}
            </h3>
            <p className="text-[#64748b] text-[15px] leading-relaxed max-w-[360px] mb-8 font-sans font-medium">
            ...
            <div className="flex flex-col gap-3 w-full max-w-[160px]">
              <Link
                ...
                className="bg-[#1a3c5a] text-white px-5 py-2.5 text-[11px] font-medium tracking-wider rounded-sm hover:bg-[#122b42] transition-colors text-center uppercase"
                onClick={() => setIsProductsOpen(false)}
              >
                View selection
              </Link>
              <Link
                href="/contact"
                className="border border-[#1a3c5a]/20 text-[#1a3c5a] bg-white px-5 py-2.5 text-[11px] font-medium tracking-wider rounded-sm hover:bg-slate-50 transition-colors text-center uppercase"
                onClick={() => setIsProductsOpen(false)}
              >
                Request demo
              </Link>
        ...
        {/* Bottom Bar */}
        <div className="bg-white border-t border-[#eee] px-8 py-4 flex justify-between items-center text-[12px] font-medium">
